  ...
  >
    ...
    <Reveal.Content hidden>
      <div className="eyeContainer">
        ...
        <div id={eye2} />
        <path
          d="M128.21,71H75.79C73.71,71,72,68.87,72,66.26v-.52C72,63.13,73.71,61,75.79,61h52.42c2.08,0,3.79,2.13,3.79,4.74v.52C132,68.87,130.29,71,128.21,71Z"
          id="brows"
        />
      </div>
    </Reveal.Content>
...
class Auth extends Component {
  ...
  handleChange(e) {
    ...
    var add = e.target.selectionStart,
      charWidth =
        parseInt(
          window.getComputedStyle(e.target).getPropertyValue("font-size")
        ) * 0.15,
      x = add * charWidth;
    ...
    var left = e.target.parentNode.getBoundingClientRect().left,
      rot1 =
        Math.atan2(
          left + x - eye.getBoundingClientRect().left,
          e.target.getBoundingClientRect().top - eye.getBoundingClientRect().top
        ) *
          (180 / Math.PI) *
          -1 +
        180,
  ...
  handleFocus(event) {
    ...
    document.body.onmousemove = function(e) {
      var rot =
          Math.atan2(
            e.pageX - eye.getBoundingClientRect().left,
            e.pageY - eye.getBoundingClientRect().top
          ) *
            (180 / Math.PI) *
            -1 +
          180,
        rot2 =
          Math.atan2(
            e.pageX - eye2.getBoundingClientRect().left,
            e.pageY - eye2.getBoundingClientRect().top
          ) *
            (180 / Math.PI) *
            -1 +
          180;
  ...
  async handleSubmit(event) {
    ...
    try {
      const res = await request({
        ...
        uri: ServerURL,
        json: true,
        body: { password: this.state.value }
      });
      if (res.auth === true) {
        this.setState({ active: true, authorized: true });
        const t = this;
  ...
  render() {
    ...
    return (
      <div>
        <Popup
          trigger={<Icon name="help" className="help circle" />}
          hoverable
          position="left center"
        >
          <Item>
            <Item.Content>
              <Item.Description>Password is: onward!</Item.Description>
            </Item.Content>
          </Item>
        </Popup>
